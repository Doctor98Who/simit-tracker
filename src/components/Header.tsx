...
const WorkoutExerciseItem: React.FC<WorkoutExerciseItemProps> = ({ 
  ...
  useEffect(() => {
    ...
    dragPreview.style.background = 'rgba(59, 130, 246, 0.9)';
    dragPreview.style.color = 'white';
    dragPreview.style.padding = '8px 16px';
    dragPreview.style.borderRadius = '8px';
    dragPreview.style.fontSize = '0.9em';
    dragPreview.style.fontWeight = '600';
    dragPreview.style.boxShadow = '0 4px 12px rgba(0,0,0,0.3)';
    dragPreview.style.position = 'fixed';
    dragPreview.style.pointerEvents = 'none';
    dragPreview.innerHTML = `${ex.name} ${ex.subtype ? `(${ex.subtype})` : ''}`;
    
    // Create a canvas to render the custom preview
    ...
    if (ctx) {
      ...
      ctx.fillText(`${ex.name} ${ex.subtype ? `(${ex.subtype})` : ''}`, canvas.width / 2, canvas.height / 2);
  ...
  const handleTouchMove = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    // If user moves finger more than 10px, cancel the long press
      ...
      {!isCollapsed && (
        <div style={{ padding: '0 12px 10px' }}>
          <div className="set-table-header" style={{ 
            display: 'grid',
            gridTemplateColumns: '30px 1fr 55px 55px 38px 24px',
            gap: '6px',
            marginBottom: '6px',
            fontSize: '0.6em',
