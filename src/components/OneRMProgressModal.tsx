...
const OneRMProgressModal: React.FC<OneRMProgressModalProps> = ({ exercise, onClose }) => {
  ...
  const yPadding = range === 0 ? maxOneRM * 0.2 : range * 0.1;
      ...
      {chartData.length === 0 ? (
        ...
        <div style={{ 
          ...
          <svg width="100%" height={chartHeight} viewBox={`0 0 ${chartWidth} ${chartHeight}`} preserveAspectRatio="xMidYMid meet">
            ...
            {/* X-axis date labels */}
            {chartDataToUse.length >= 1 && (chartDataToUse.length === 1 
              ? [0].map(i => {
              ...
              : [0, Math.floor(chartDataToUse.length / 2), chartDataToUse.length - 1].map(i => {
            ...
            {chartDataToUse.map((point, index) => {
              const x = padding.left + ((point.date - dateRange.min) / (dateRange.max - dateRange.min)) * plotWidth;
              const y = chartHeight - padding.bottom - ((point.estimated1RM - paddedMin) / (paddedMax - paddedMin)) * plotHeight;
                ...
                <g key={index}>
                  {index > 0 && (
                    <line
                      ...
                      y1={chartHeight - padding.bottom - ((chartDataToUse[index - 1].estimated1RM - paddedMin) / (paddedMax - paddedMin)) * plotHeight}
                      x2={x}
                      y2={y}
